...
import { getPage } from '../workspace/pages';
...
export async function executeAction(action: AgentAction): Promise<{ success: boolean; action: AgentAction }> {
  ...
  try {
    switch (action.type) {
      ...
      case "create_page": {
        const pagesApi = await import('../workspace/pages');
        const { title, parentId, blocks } = action.payload || {};
        if (!title) throw new Error("title required for create_page");
        const created = await pagesApi.createPage(title, parentId, Array.isArray(blocks) ? blocks : []);
        savePreviousState(action, { createdPageId: created.id });
        break;
      }
      ...
      case "update_page": {
        ...
        if (!pageId || !updates) throw new Error("pageId and updates required for update_page");
        const original = await pagesApi.getPage(pageId);
        if (original) savePreviousState(action, original);
        await pagesApi.updatePageMetadata(pageId, updates);
        break;
      }
      ...
      case "delete_page": {
        ...
        if (!pageId) throw new Error("pageId required for delete_page");
        const original = await pagesApi.getPage(pageId);
        if (original) savePreviousState(action, original);
      ...
      case "modify_page": {
        ...
        if (originalPage) savePreviousState(action, originalPage);
